...
def activity_campaignProposal_executeReport(settings: dict):
    ...
    mover_totals = pd.read_csv(BytesIO(blob_client.download_blob().content_as_bytes())).iloc[0]
    
    # import mover counts
    blob_client = container_client.get_blob_client(blob=f"{settings['instance_id']}/mover_counts.csv")
    mover_counts = pd.read_csv(BytesIO(blob_client.download_blob().content_as_bytes()))

    # import competitor list
    blob_client = container_client.get_blob_client(blob=f"{settings['instance_id']}/competitors.csv")
    competitors = pd.read_csv(BytesIO(blob_client.download_blob().content_as_bytes()))
...
def execute_graphics_replacements(template:Presentation, settings:dict, container_client:ContainerClient, resources_client:ContainerClient):
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[24],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[27],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[26],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[25],
    )  
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[28],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=display_social_slide,
        placeholder=display_social_slide.shapes[29],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=ott_slide,
        placeholder=ott_slide.shapes[15],
    )
    ...
    add_custom_image(
        file=BytesIO(blob_client.download_blob().content_as_bytes()),
        slide=ott_slide,
        placeholder=ott_slide.shapes[17],
    )