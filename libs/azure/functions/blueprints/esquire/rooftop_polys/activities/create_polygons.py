...
import googlemaps, logging, numpy as np, os, pandas as pd
...
def activity_rooftopPolys_createPolygons(queries: dict):
    ...
    if len(gmaps_passed):
        ...
        if len(roads_passed) and "polygon" in roads_passed.columns:
            roads_passed["geojson"] = roads_passed["polygon"].apply(
                lambda x: mapping(x)
            )
...
def execute_google_maps_call(
    gmaps: googlemaps.Client,
    query_list: list,
    min_frame_area: int = 25,
    max_frame_area: int = 3000,
) -> pd.DataFrame:
    """
    ...
    for i, query in enumerate(query_list):
        ...
        if len(
            res
        ):  # skip if response is an empty list (rare case if couldn't find a match)
            gmaps_results_list.append({"index": i, "query": query, "result": res[0]})

    gmaps_results = pd.json_normalize(gmaps_results_list)
    if (
        len(gmaps_results)
        and "result.geometry.bounds.northeast.lng" in gmaps_results.columns
    ):
        gmaps_results = gmaps_results.dropna(
            subset=["result.geometry.bounds.northeast.lng"]
        )
        ...
        gmaps_results["polygon"] = gmaps_results.apply(
            lambda row: Polygon(
                [
                    [
                        row["result.geometry.bounds.northeast.lng"],
                        row["result.geometry.bounds.northeast.lat"],
                    ],
                    [
                        row["result.geometry.bounds.northeast.lng"],
                        row["result.geometry.bounds.southwest.lat"],
                    ],
                    [
                        row["result.geometry.bounds.southwest.lng"],
                        row["result.geometry.bounds.southwest.lat"],
                    ],
                    [
                        row["result.geometry.bounds.southwest.lng"],
        ...
        gmaps_passed = gmaps_results[
            gmaps_results["area"].between(min_frame_area, max_frame_area)
        ].copy()
...
def execute_roads_call(
    gmaps: googlemaps.Client, df: pd.DataFrame, road_buffer: int = 3
) -> pd.DataFrame:
    """
    ...
    for name, g_chunk in df.groupby(np.arange(len(df)) // 50):
        ...
        points = [
            [x["result.geometry.location.lat"], x["result.geometry.location.lng"]]
            for i, x in g_chunk.iterrows()
        ]
        ...
        for h_idx, house in g_chunk.iterrows():
            ...
            for r_idx, road in road_candidates.iterrows():
                road_circle = latlon_circle(
                    lat=road["location.latitude"],
                    lon=road["location.longitude"],
                    radius=road_buffer,
                )
