...
import geojson, orjson as json


def polygon_splitting_wrapper(
    feature, axis="minor", max_area=2.8, export_path=False, prints=False
):
    """
    ...
    if export_path:
        geojson_stringify_collection(
            splits, main_name=main_name, export_path=export_path
        )
    else:
    ...
    if polygon_area(polygon) <= max_area:
        ...
        for poly in polys.geoms:
            ...
            split_geoms = split_geoms + [
                split_poly
                for split_poly in split_recursive(
                    poly, axis=axis, max_area=max_area
                ).geoms
            ]
...
def geojson_stringify_collection(
    geometry_collection, main_name="Polygon", export_path=False
):
    """
    ...
    if len(geometry_collection.geoms) > 1:
        GeoJSON_string = geojson.FeatureCollection(
            features=[
                geojson.Feature(
                    geometry=poly, properties={"name": f"{main_name}{ii+1}"}
                )
                for ii, poly in enumerate(geometry_collection.geoms)
            ]
        )
    else:
    ...
    if export_path:
        with open(f"{export_path}/{main_name}.geojson", "w") as outfile:
            outfile.write(json.dumps(GeoJSON_string, indent=2))
    else:
        return geojson.dumps(GeoJSON_string, indent=2)
