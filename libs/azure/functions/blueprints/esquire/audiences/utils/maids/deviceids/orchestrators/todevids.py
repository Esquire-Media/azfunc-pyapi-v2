...
def orchestrator_esquireAudienceMaidsDeviceIds_todevids(
    ...
    suborchestrator to process device IDs and then merges the resulting device id files.
            ...
            context.call_sub_orchestrator_with_retry(
                ...
                retry,
                {
                    **ingress["working"],
                    ...
                    "request": json.loads(
                        download_blob_bytes(ingress["source"])
                    ),
                },
            )
        ]
