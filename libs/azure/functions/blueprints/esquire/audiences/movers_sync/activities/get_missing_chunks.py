...
def activity_moversSync_getMissingChunks(settings: dict):
    """
    Identifies missing chunks of data that require address validation.

    This function checks Azure Table Storage for expected row counts and compares it with
    the data present in Azure Blob Storage to identify missing chunks. It returns a list
    of these missing chunks, formatted for subsequent address validation.

    Parameters
    ----------
    settings : dict
        Dictionary containing settings for Azure Blob and Table Storage connections.
        The expected keys and their values are:
        - 'runtime_container': A dictionary with connection details for the runtime container. It should contain:
            - 'conn_str': The name of the environment variable holding the connection string.
            - 'container_name': The name of the Azure Blob container.
        - 'rowCounts_table': A dictionary with connection details for the table holding row counts. It should contain:
            - 'conn_str': The name of the environment variable holding the connection string.
            - 'table_name': The name of the Azure Table.
    ...
    row_counts["chunk_blob_type"] = row_counts["PartitionKey"].apply(
        lambda x: f"{x}-geocoded"
    )
    # only enforce address validation on the most recent 24 weeks (~6 months) of data
    row_counts["Date"] = (
        row_counts["RowKey"]
        .apply(lambda x: re.search("[0-9]{4}_[0-9]{2}_[0-9]{2}", x)[0])
        .apply(lambda x: dt.strptime(x, "%Y_%m_%d"))
    )
    ...
    chunk_blobs = pd.DataFrame(
        [
            {k: v for k, v in blob.items() if k in ["name", "container"]}
            for blob in blobs
        ]
    )
    ...
    for blob_keys, blob_df in merged.groupby(["PartitionKey", "RowKey"]):
        ...
        for i, row in blob_df[~blob_df["chunk_name"].isnull()].iterrows():
            capture_groups = re.search(
                pattern="offset=(\d+),limit=(\d+)", string=row["chunk_name"]
            )
        ...
        adjusted_missing_ranges = adjust_range_length(
            missing_ranges,
            max_range_length=int(os.environ["ADDRESS_VALIDATION_CHUNK_SIZE"]),
        )
