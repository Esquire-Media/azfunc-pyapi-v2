...
def activity_campaignProposal_collectCompetitors(settings: dict):
    ...
    in_client = container_client.get_blob_client(
        blob=f"{settings['instance_id']}/addresses.csv"
    )
    addresses = pd.read_csv(
        get_blob_sas(in_client), usecols=["address", "latitude", "longitude"]
    )
    ...
    comps["closed_bucket_value"] = index_by_list(
        comps["closed_bucket"],
        sort_list=[
            "VeryLikelyClosed",
            "LikelyClosed",
    ...
    comps["venue_reality_bucket_value"] = index_by_list(
        comps["venue_reality_bucket"],
        sort_list=["VeryLow", "Low", "Medium", "High", "VeryHigh"],
    )
    comps["reality_score"] = (
        comps["closed_bucket_value"] + comps["venue_reality_bucket_value"]
    )
    ...
    out_client = container_client.get_blob_client(
        blob=f"{settings['instance_id']}/competitors.csv"
    )
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        unique = distances.sort_values(
            "distance_miles", ascending=True
        ).drop_duplicates(subset=["fsq_id"], keep="first")
    ...
    out_client = container_client.get_blob_client(
        blob=f"{settings['instance_id']}/Competitors-{settings['name']}.xlsx"
    )
    out_client.upload_blob(output.getvalue(), overwrite=True)

    # COMPETITOR SCATTERMAPS
    bytes = map_competitors(
        comps=comps, owned=addresses, mapbox_token=mapbox_token, return_bytes=True
    )
    out_client = container_client.get_blob_client(
        blob=f"{settings['instance_id']}/competitors.png"
    )
